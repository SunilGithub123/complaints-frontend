 ...
 * `errors.<code>` key in `@complaints/i18n`. Twin of
 * `apps/web/src/lib/apiErrors.ts` — same contract, same fallback
 * ladder, same `fieldErrors` pass-through.
 *
 ...
 *
 * Kept in mobile-local code rather than promoted to `@complaints/utils`
 * because it depends on `@complaints/api`'s `ApiError` (which depends
 * on the generated client). Promoting would require dragging the api
